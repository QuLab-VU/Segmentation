function percent = parfor_progress(N,t,i,num_cores)
...
%     end
    ...
    progress = fscanf(f, '%d,%f',[2 Inf])';
    fclose(f);
    percent = (length(progress)-1)/progress(1,1)*100;
    est_time = (mean(progress(2:end,2))*(progress(1,1)-length(progress)-1))/60/num_cores; %Divide by number of Cores
    if nargout == 0
        perc = sprintf('%3.0f%% Est. Time(min):%.2f', percent,est_time); % 4 characters wide, percentage
        disp(perc);

...
for i = 1:handles.numCh:size(unfinishedImages,2)
    tic
    ...
    save([handles.expDir filesep 'Segmented' handles.startdate filesep CO.filename '.mat'], 'CO')
    t(i) = toc;
    fprintf('%.2f%% Complete, Estimated Time: %.2f\n',i/length(unfinishedImages)*100,mean(t)/60*(length(unfinishedImages)-i))
end
fid = fopen([handles.expDir filesep 'Segmented' handles.startdate filesep 'NumDetect_Im.txt'], 'w');

function [] = reDrawBorders(idx,im_cell_id,classLabel,ax1,ax2,CO,num_nuc,num_apo,num_over,num_under,num_mito)

axes(ax1)
cla;
load(seg_file(idx).name)
im = imread(CO.filename);
if size(im,3)~=1
    im = rgb2gray(im);
end
im = im./max(im(:));
...
properties = regionprops(CO.Nuc_label,'BoundingBox');
for p = 1:length(properties)
    ...
    [~,loc] = ismember([idx,CO.cellId(p)],im_cell_id,'rows');
    if loc~=0

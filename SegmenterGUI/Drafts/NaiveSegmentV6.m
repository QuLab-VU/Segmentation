function [CO,Im_array] = NaiveSegmentV6(imExt,cidrecorrect,NucnumLevel,CytonumLevel,surface_segment,nuclear_segment,noise_disk,nuc_noise_disk,...
...
if numCh >0
    ...
    for q = 1:numCh
        ...
        [FeatureSet, FeatureSetVarLabels] = CellFeature_reduced(Im_array(:,:,q+1),CytoLabel,1:size(p,1),[]);
        CO.(chnm).Circularity=FeatureSet(:,3);
        CO.(chnm).Hu_mom1=FeatureSet(:,4);
        CO.(chnm).Hu_mom2=FeatureSet(:,5);
        CO.(chnm).Hu_mom3=FeatureSet(:,6);
        CO.(chnm).Hu_mom4=FeatureSet(:,7);
        CO.(chnm).Hu_mom5=FeatureSet(:,8);
        CO.(chnm).Hu_mom6=FeatureSet(:,9);
        CO.(chnm).Hu_mom7=FeatureSet(:,10);
        CO.(chnm).Extension = FeatureSet(:,11);
        CO.(chnm).Dispersion= FeatureSet(:,12);
        CO.(chnm).Elongation =FeatureSet(:,13);
        CO.(chnm).Mean_Dist_to_closest_objs=FeatureSet(:,14);
        CO.(chnm).Mean_Pixel_Dist=FeatureSet(:,15);
        CO.(chnm).Max_Pixel_Dist=FeatureSet(:,16);
        CO.(chnm).Min_Pixel_Dist=FeatureSet(:,17);
        CO.(chnm).Std_Pixel_Dist=FeatureSet(:,18);
        CO.(chnm).Min_Intensity=FeatureSet(:,20);
        CO.(chnm).Max_Intensity=FeatureSet(:,21);
        CO.(chnm).Std_Intensity=FeatureSet(:,22);
        CO.(chnm).Mean_Gradient_Full=FeatureSet(:,23);
        CO.(chnm).Min_Gradient_Full=FeatureSet(:,24);
        CO.(chnm).Max_Gradient_Full=FeatureSet(:,25);
        CO.(chnm).Std_Gradient_Full=FeatureSet(:,26);
        end
    end
    Edge = [];
    if size(p,1) ~= 0

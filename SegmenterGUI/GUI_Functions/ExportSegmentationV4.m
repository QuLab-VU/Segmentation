...
for i = 1:size(seg_file,1)
    ...
    if double(CO.cellCount) ~=0
        ...
        for q = 1:handles.numCh
            chnm = ['CH_' num2str(q)];
            if q == 1
                Seg.Cyto.Area(cnt:cnt2)         = CO.(chnm).Area;
                Seg.Cyto.Perimeter(cnt:cnt2)    = CO.(chnm).Perimeter;
                Seg.Cyto.Circularity(cnt:cnt2)  = CO.(chnm).Circularity;
                Seg.Cyto.Hu_mom1(cnt:cnt2)      = CO.(chnm).Hu_mom1;
                Seg.Cyto.Hu_mom2(cnt:cnt2)      = CO.(chnm).Hu_mom2;
                Seg.Cyto.Hu_mom3(cnt:cnt2)      = CO.(chnm).Hu_mom3;
                Seg.Cyto.Hu_mom4(cnt:cnt2)      = CO.(chnm).Hu_mom4;
                Seg.Cyto.Hu_mom5(cnt:cnt2)      = CO.(chnm).Hu_mom5;
                Seg.Cyto.Hu_mom6(cnt:cnt2)      = CO.(chnm).Hu_mom6;
                Seg.Cyto.Hu_mom7(cnt:cnt2)      = CO.(chnm).Hu_mom7;
                Seg.Cyto.Extension(cnt:cnt2)    = CO.(chnm).Extension;
                Seg.Cyto.Dispersion(cnt:cnt2)   = CO.(chnm).Dispersion;
                Seg.Cyto.Elongation(cnt:cnt2)   = CO.(chnm).Elongation;
            end

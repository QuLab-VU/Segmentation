function [FeatureSet, FeatureSetVarLabels] = CellFeature_reduced(orig_im,label_im,idx,properties)
    num_ch = size(orig_im,3);  %Number of parameters
    ...
    %Pre-allocate vectors holding pixel info.
    ...
    %Pre-allocate FeatureSet Vector
    ...
    for i =  1:length(idx)        
        if properties(idx(i)).Area ~=0
            ...
            for j = 1:num_ch
                ...
                G_f(j,:) = [mean(tempIm(idx_rel)), min(tempIm(idx_rel)), max(tempIm(idx_rel)),std(tempIm(idx_rel))];
            end

            %Put it all together
            G_f = G_f';
            FeatureSet(i,:) = [A,P,C,H',Ext,Dis,El,D,meanPD,maxPD,minPD,stPD,...
                Mean_I',Min_I',Max_I',Std_I',G_f(:)'];
        else
    ...
    for i = 1:num_ch
        ...
        str2 = sprintf('Min_Gradient_Full_CH%i',i);
        str3 = sprintf('Max_Gradient_Full_CH%i',i);
        str4 = sprintf('Std_Gradient_Full_CH%i',i);
        FeatureSetVarLabels = [FeatureSetVarLabels, str1,str2,str3,str4];
    end

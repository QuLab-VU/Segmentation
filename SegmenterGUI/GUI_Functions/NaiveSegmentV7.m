function [CO,Im_array] = NaiveSegmentV7(cidrecorrect,NucnumLevel,CytonumLevel,surface_segment,...
...
%Get the properties of each segmented cells for each channel and Nucleus
    ...
    if size(p,1) ~=0
        ...
        for q = 1:num_ch
            ...
            CO.(chnm).Mean_Pixel_Dist   = FeatureSet(:,15+q-1);
            CO.(chnm).Max_Pixel_Dist    = FeatureSet(:,16+num_ch+q-1);
            CO.(chnm).Min_Pixel_Dist    = FeatureSet(:,17+num_ch*2+q-1);
            CO.(chnm).Std_Pixel_Dist    = FeatureSet(:,18+num_ch*3+q-1);
            CO.(chnm).Mean_Intensity    = FeatureSet(:,19+num_ch*4+q-1);
            CO.(chnm).Min_Intensity     = FeatureSet(:,20+num_ch*5+q-1);
            CO.(chnm).Max_Intensity     = FeatureSet(:,21+num_ch*6+q-1);
            CO.(chnm).Std_Intensity     = FeatureSet(:,22+num_ch*7+q-1);
            CO.(chnm).Mean_Gradient_Full= FeatureSet(:,23+num_ch*8+q-1);
            CO.(chnm).Min_Gradient_Full = FeatureSet(:,24+num_ch*9+q-1);
            CO.(chnm).Max_Gradient_Full = FeatureSet(:,25+num_ch*10+q-1);
            CO.(chnm).Std_Gradient_Full = FeatureSet(:,26+num_ch*11+q-1);
        end
        CO.label            = CytoLabel;
